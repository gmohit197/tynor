...
class DoctorFragment: Executeapi, UITableViewDataSource,UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return mycustomeradapter.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! DoctorFragmentcell
    ...
    override func viewDidLoad() {
        ...
        doctor.leftViewMode = UITextFieldViewMode.always
        let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 25, height: 18))
        let image = UIImage(named: "search1.png")
        imageView.image = image
        doctor.leftView = imageView
        self.doctortable.tableFooterView = UIView()
        ...
        customerspinr.didSelect { (selected, index, id) in
            ...
            if self.usercode == ""
            {
                self.getRetailerListDetail(query: self.doctor.text, customertype: "CG0003", keycustomer: "", cityname: "")
            }
            else{
                self.getRetailerListDetail(query: self.doctor.text, customertype: "CG0003", keycustomer: "", cityname: "", uc: self.usercode!)
            }
        }
        
        getRetailerListDetail(query: "", customertype: "CG0003", keycustomer: "", cityname: "")
        setspinr()
    ...
    @IBAction func doctorsearch(_ sender: Any) {
        if self.usercode == ""{
            getRetailerListDetail(query: self.doctor.text, customertype: "CG0003", keycustomer: "", cityname: "")
        }
        else{
            self.getRetailerListDetail(query: self.doctor.text, customertype: "CG0003", keycustomer: "", cityname: "", uc: self.usercode)
        }
