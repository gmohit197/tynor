...
class RetailerFragment: Executeapi, UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return mycustomeradapter.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! RetailerFragmentcell
    ...
    @IBOutlet weak var retailer: UITextField!
    @IBOutlet weak var customerspinr: DropDownUtil!
    @IBOutlet weak var reatilertable: UITableView!
    ...
    override func viewDidLoad() {
        ...
        customerspinr.didSelect { (selected, index, id) in
            ...
            if self.usercode == ""
            {
                self.getRetailerListDetail(query: self.retailer.text, customertype: "CG0001", keycustomer: "", cityname: "")
            }
            else{
                self.getRetailerListDetail(query: self.retailer.text, customertype: "CG0001", keycustomer: "", cityname: "", uc: self.usercode!)
            }
        }
        getRetailerListDetail(query: "", customertype: "CG0001", keycustomer: "", cityname: "")
        setspinr()
    ...
    public func getRetailerListDetail (query: String?, customertype: String? , keycustomer: String?, cityname: String?) {
     ...
     func setspinr()
     {
        ...
        var stmt1: OpaquePointer?
        let query = "select usercode,empname from USERHIERARCHY where usertype = '11' or usertype = '12'"
    ...
    @IBAction func retailerSearch(_ sender: UITextField) {
        if self.usercode == ""
               {
                   getRetailerListDetail(query: self.retailer.text, customertype: "CG0001", keycustomer: "", cityname: "")
               }
               else
               {
                   self.getRetailerListDetail(query: self.retailer.text, customertype: "CG0001", keycustomer: "", cityname: "", uc: self.usercode!)
