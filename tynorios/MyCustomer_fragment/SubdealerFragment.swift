...
class SubdealerFragment: Executeapi, UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return mycustomeradapter.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! SubdealerFragmentcell
    ...
    @IBOutlet weak var subdealer: UITextField!
    ...
    override func viewDidLoad() {
        ...
        subdealer.leftViewMode = UITextFieldViewMode.always
        let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 25, height: 18))
        let image = UIImage(named: "search1.png")
        imageView.image = image
        subdealer.leftView = imageView
        self.subdealertable.tableFooterView = UIView()
       ...
       // nodata.isHidden = true
        ...
        customerspinr.didSelect { (selected, index, id) in
            ...
            if self.usercode == ""
            {
                self.getRetailerListDetail(query: self.subdealer.text, customertype: "CG0004", keycustomer: "", cityname: "")
            }
            else{
                self.getRetailerListDetail(query: self.subdealer.text, customertype: "CG0004", keycustomer: "", cityname: "", uc: self.usercode!)
            }
        }
        
        getRetailerListDetail(query: "", customertype: "CG0004", keycustomer: "", cityname: "")
        setspinr()
    ...
    @IBAction func subdealersearch(_ sender: Any) {
    ...
    @IBAction func SubDealerSearch(_ sender: UITextField) {
        if self.usercode == ""{
                   getRetailerListDetail(query: self.subdealer.text, customertype: "CG0004", keycustomer: "", cityname: "")
               }
               else{
                   self.getRetailerListDetail(query: self.subdealer.text, customertype: "CG0004", keycustomer: "", cityname: "", uc: self.usercode!)
               }
